...
# pytorch3d is hard to install on some platforms; use torch.cdist for KNN
...
class GaussianModel(nn.Module):
    ...
    def _init_scales(self, points3D_xyz: torch.Tensor) -> None:
        ...
        # Compute mean distance to K nearest neighbors (using torch.cdist)
        K = min(50, self.n_points - 1)
        # Compute pairwise Euclidean distances: (N, N)
        pairwise_dists = torch.cdist(points3D_xyz, points3D_xyz, p=2)
        # Sort and take K nearest (skip first: distance to self = 0)
        sorted_dists, _ = torch.sort(pairwise_dists, dim=1)
        knn_dists = sorted_dists[:, 1:K+1]  # exclude self

        # Use log space for unconstrained optimization
        mean_dists = torch.mean(knn_dists, dim=1, keepdim=True) * 2.
    ...
    def compute_covariance(self) -> torch.Tensor:
        ...
        # Compute covariance: Σ = R S Sᵀ Rᵀ  (Eq. 6 in the paper)
        # Since S is diagonal, S = Sᵀ, and S @ S gives element-wise squares
        Covs3d = R @ S @ S.transpose(-1, -2) @ R.transpose(-1, -2)  # (N, 3, 3)
