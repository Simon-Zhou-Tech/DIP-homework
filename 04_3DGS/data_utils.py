...
class ColmapDataset(Dataset):
    def __init__(self, data_path, downsample_factor=8, maximum_pts_num = 5000):
        """
        ...
        # Sample 3D points to a manageable number for CPU training
        if len(self.points3D_xyz) > maximum_pts_num:
            n_pts = len(self.points3D_xyz)
            indices = torch.randperm(n_pts)[:maximum_pts_num]
            self.points3D_xyz = self.points3D_xyz[indices]
            self.points3D_rgb = self.points3D_rgb[indices]
            print(f'Sampled {maximum_pts_num} / {n_pts} 3D points')
