...
class GaussianRenderer(nn.Module):
    ...
    def compute_projection(
        ...
        # Transform covariance to camera space: Σ_cam = W Σ_world Wᵀ  (Eq. 5)
        # W is the world-to-camera rotation matrix R
        covs_cam = R @ covs3d @ R.transpose(-1, -2)  # (N, 3, 3)
    ...
    def compute_gaussian_values(
        ...
        # det(Σ) for 2x2 matrix: a*d - b*c
        det = covs2D[:, 0, 0] * covs2D[:, 1, 1] - covs2D[:, 0, 1] * covs2D[:, 1, 0]  # (N,)
        det = det.clamp(min=1e-8)

        # Compute inverse of 2x2 covariance for the quadratic form
        # inv(Σ) = 1/det * [[d, -b], [-c, a]]
        ...
        # Quadratic form: (x-μ)ᵀ Σ⁻¹ (x-μ) for each Gaussian at each pixel
        # dx: (N, H, W, 2), inv_covs2D: (N, 2, 2)
        power = -0.5 * torch.einsum('nhwi,nij,nhwj->nhw', dx, inv_covs2D, dx)  # (N, H, W)

        # Normalization: 1 / (2π √|Σ|)
        norm = 1.0 / (2.0 * np.pi * torch.sqrt(det))  # (N,)

        gaussian = norm.view(-1, 1, 1) * torch.exp(power)  # (N, H, W)
    ...
    def forward(
        ...
        # 7. Compute alpha-blending weights: w_i = α_i * Π_{j<i} (1 - α_j)
        alphas = alphas.clamp(0.0, 1.0)  # numerical stability
        T = torch.cumprod(1.0 - alphas, dim=0)  # (N, H, W): T[i] = Π_{j≤i} (1-α_j)
        # T_i = Π_{j<i} (1-α_j) — shift so T_0 = 1 (product over empty set)
